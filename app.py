...
app.py — AFib AI
================
Streamlit app for AFib detection using XGBoost and CatBoost.
Medical white/green theme. Sliding window demo on MITB data.
"""
...
# ============================================================
# COLOR PALETTE — Medical white/green theme
# ============================================================
COLORS = {
    "bg":           "#f0f4f8",
    "panel":        "#ffffff",
    ...
    "warn_light":   "#fef3e2",
    "ecg_bg":       "#ffffff",
    "ecg_grid_maj": "rgba(210,50,50,0.25)",
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  [data-testid="stSidebar"] {{ background: {COLORS["panel"]} !important; border-right: 1px solid {COLORS["border"]} !important; box-shadow: 2px 0 8px rgba(0,0,0,0.06) !important; }}
  ...
  .stTabs [data-baseweb="tab-list"] {{ background: {COLORS["panel"]}; border-bottom: 2px solid {COLORS["border"]}; padding: 0 1.5rem; }}
  .stTabs [data-baseweb="tab"] {{ color: {COLORS["text_mid"]} !important; font-size: 0.78rem !important; font-weight: 600 !important; letter-spacing: 0.06em !important; text-transform: uppercase !important; padding: 0.9rem 1.4rem !important; border-bottom: 2px solid transparent !important; margin-bottom: -2px !important; background: transparent !important; }}
  .stTabs [aria-selected="true"] {{ color: {COLORS["accent"]} !important; border-bottom: 2px solid {COLORS["accent"]} !important; }}
  .stTabs [data-baseweb="tab-panel"] {{ padding: 1.5rem 0 !important; background: {COLORS["bg"]}; }}

  [data-testid="metric-container"] {{ background: {COLORS["panel"]}; border: 1px solid {COLORS["border"]}; border-radius: 10px; padding: 1rem !important; box-shadow: 0 1px 4px rgba(0,0,0,0.06); }}
  [data-testid="stMetricValue"] {{ font-family: 'JetBrains Mono', monospace !important; font-size: 1.5rem !important; color: {COLORS["text"]} !important; font-weight: 600 !important; }}
  [data-testid="stMetricLabel"] {{ font-size: 0.65rem !important; font-weight: 700 !important; letter-spacing: 0.1em !important; text-transform: uppercase !important; color: {COLORS["text_dim"]} !important; }}

  .cs-alert {{ border-radius: 10px; padding: 1rem 1.3rem; margin: 0.6rem 0; display: flex; align-items: flex-start; gap: 0.8rem; }}
  .cs-alert-afib     {{ background: {COLORS["danger_light"]}; border: 1px solid rgba(192,57,43,0.3); border-left: 4px solid {COLORS["danger"]}; }}
  .cs-alert-normal   {{ background: {COLORS["accent_light"]}; border: 1px solid rgba(10,124,78,0.3); border-left: 4px solid {COLORS["accent"]}; }}
  ...
  .cs-card  {{ background: {COLORS["panel"]}; border: 1px solid {COLORS["border"]}; border-radius: 12px; padding: 1.4rem; margin-bottom: 0.8rem; box-shadow: 0 1px 4px rgba(0,0,0,0.06); }}
  ...
  .stButton>button {{ background: linear-gradient(135deg, {COLORS["accent"]}, {COLORS["accent2"]}) !important; color: white !important; border: none !important; border-radius: 8px !important; font-weight: 600 !important; padding: 0.5rem 1.4rem !important; box-shadow: 0 2px 6px rgba(10,124,78,0.25) !important; }}
  .stButton>button:hover {{ opacity: 0.9; }}
  .stDownloadButton > button {{ background: linear-gradient(135deg, {COLORS["accent"]}, {COLORS["accent2"]}) !important; color: white !important; border: none !important; border-radius: 8px !important; font-weight: 600 !important; font-size: 0.8rem !important; }}
  [data-testid="stFileUploader"] {{ background: {COLORS["panel"]}; border: 1.5px dashed {COLORS["border"]}; border-radius: 10px; }}
  .stDataFrame {{ border: 1px solid {COLORS["border"]} !important; border-radius: 8px !important; overflow: hidden; }}
  .streamlit-expanderHeader {{ background: {COLORS["panel"]} !important; border: 1px solid {COLORS["border"]} !important; border-radius: 8px !important; color: {COLORS["text"]} !important; font-size: 0.82rem !important; }}
...
def hrv_heuristic(features):
    ...
    score = 0.0; reasons = {}
    sdnn_n  = min(feat["sdnn"]/150.0, 1.0);      reasons["SDNN"]            = sdnn_n;  score += sdnn_n*0.25
    pnn50_n = min(feat["pnn50"]/0.5, 1.0);        reasons["pNN50"]           = pnn50_n; score += pnn50_n*0.20
    irr_n   = min(feat["irr_score"]/0.4, 1.0);    reasons["Irregularity"]    = irr_n;   score += irr_n*0.25
    rmssd_n = min(feat["rmssd"]/120.0, 1.0);      reasons["RMSSD"]           = rmssd_n; score += rmssd_n*0.15
    lfhf_i  = max(0.0,1.0-min(feat["lf_hf"]/2.0,1.0)); reasons["LF/HF"]    = lfhf_i;  score += lfhf_i*0.10
    cv_n    = min(feat["cv_rr"]/0.25, 1.0);       reasons["CV(RR)"]          = cv_n;    score += cv_n*0.05
    prob = float(np.clip(score, 0.0, 1.0))
    return ("AFib" if prob >= 0.45 else "Normal"), prob, 0.45, reasons
...
def plot_ecg(signal, peaks, fs=FS, title="ECG Signal", is_afib=False, window_start=0):
    max_pts = 2000
    step    = max(1, len(signal) // max_pts)
    disp    = signal[::step]
    t       = np.arange(len(disp)) * step / fs + window_start
    ...
    if len(peaks):
        ...
        fig.add_trace(go.Scatter(
            x=valid / fs + window_start, y=signal[valid], mode="markers",
    ...
    fig.update_layout(
        **_base_layout(height=300, plot_bgcolor=COLORS["ecg_bg"]),
        title=dict(text=title, font=dict(family="Inter", size=12,
                   color=COLORS["text_mid"]), x=0.01),
        xaxis=dict(
...
def plot_rr(rr_ms):
    ...
    fig.add_hline(y=m, line_dash="dash", line_color=COLORS["warn"], opacity=0.8,
                  annotation_text=f"Mean: {m:.0f}ms",
                  annotation_font=dict(color=COLORS["warn"], size=10))
    fig.add_hrect(y0=600, y1=1000, fillcolor="rgba(10,124,78,0.05)", line_width=0,
                  annotation_text="Normal range",
                  annotation_position="top left",
                  annotation=dict(font_color=COLORS["accent"], font_size=11))
    fig.update_layout(
        **_base_layout(height=260),
        title=dict(text="RR Interval Series",
                   font=dict(family="Inter", size=12, color=COLORS["text_mid"])),
        xaxis=dict(title="Beat #", color=COLORS["text_mid"],
                   gridcolor=COLORS["border"],
                   tickfont=dict(family="JetBrains Mono", size=10)),
        yaxis=dict(title="RR (ms)", color=COLORS["text_mid"],
                   gridcolor=COLORS["border"],
                   tickfont=dict(family="JetBrains Mono", size=10)),
    )
    return fig
...
def plot_poincare(rr_ms, is_afib=False):
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=COLORS["border"], dash="dash", width=1), showlegend=False,
    ))
    fig.update_layout(
        **_base_layout(height=260),
        title=dict(text="Poincaré Plot",
                   font=dict(family="Inter", size=12, color=COLORS["text_mid"])),
        xaxis=dict(title="RRₙ (ms)", color=COLORS["text_mid"],
                   gridcolor=COLORS["border"], range=lim,
                   tickfont=dict(family="JetBrains Mono", size=10)),
        yaxis=dict(title="RRₙ₊₁ (ms)", color=COLORS["text_mid"],
                   gridcolor=COLORS["border"], range=lim,
                   tickfont=dict(family="JetBrains Mono", size=10)),
    )
    return fig
...
def plot_gauge(prob):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge=dict(
            ...
            steps=[
                dict(range=[0,   35], color="rgba(10,124,78,0.08)"),
                dict(range=[35,  65], color="rgba(217,119,6,0.08)"),
                dict(range=[65, 100], color="rgba(192,57,43,0.08)"),
            ],
...
def plot_radar(features):
    ...
    fig = go.Figure(go.Scatterpolar(
        ...
        fillcolor="rgba(10,124,78,0.10)", line=dict(color=COLORS["accent"], width=2),
    ))
    fig.update_layout(
        polar=dict(
...
def main():
    ...
    with st.sidebar:
        ...
        <div style='padding:1rem 0 0.8rem;'>
          ...
          <div style='font-family:"Sora",sans-serif; font-size:1.3rem;
                      color:{COLORS["text"]}; font-weight:700; line-height:1;'>AFib AI</div>
        ...
        if input_mode == "Demo ECG":
            demo_choice = st.selectbox("Demo ECG", list(DEMO_FILES.keys()))

        st.divider()
        st.markdown(f'<div class="cs-label">Signal Settings</div>', unsafe_allow_html=True)
        fs_input     = st.number_input("Sampling rate (Hz)", min_value=64,
                                       max_value=2000, value=128, step=1)
        ...
        if XGB_AVAILABLE:
            ...
            st.markdown(f'<div class="cs-badge">🔴 xgboost not installed</div>',
                        unsafe_allow_html=True)
        if CATBOOST_AVAILABLE:
            MODEL_OPTIONS.append("CatBoost")
        else:
            st.markdown(f'<div class="cs-badge">🔴 catboost not installed</div>',
                        unsafe_allow_html=True)
        if not MODEL_OPTIONS:
            MODEL_OPTIONS = ["HRV Heuristic"]
        ...
        for name, ok in [("XGBoost", XGB_AVAILABLE), ("CatBoost", CATBOOST_AVAILABLE)]:
            dot = "🟢" if ok else "🔴"
            st.markdown(f'<div class="cs-badge">{dot} {name}</div>',
                        unsafe_allow_html=True)
    ...
    # ── TOP BAR ──────────────────────────────────────────────────────────
    st.markdown(f"""
    <div style='background:{COLORS["panel"]}; border-bottom:2px solid {COLORS["border"]};
      ...
      <div style='display:flex; align-items:center; gap:12px;'>
        ...
        <div>
          <span style='font-family:"Sora",sans-serif; font-size:1.25rem;
                       color:{COLORS["text"]}; font-weight:700;'>
            AFib AI
          </span>
          <span style='font-size:0.72rem; color:{COLORS["text_dim"]}; margin-left:10px;
    ...
    if input_mode == "Demo ECG":
        try:
            ...
            if demo_choice == "Normal #3":
                st.info("This sample is intentionally challenging — HRV characteristics "
                        "may resemble AFib despite being labelled Normal.")
        except FileNotFoundError:
            st.error(f"Demo file not found at `{DEMO_FILES[demo_choice]['path']}`.")
            st.stop()
        ...
        if uploaded_file is not None:
            ...
            if data.ndim == 2:
                ...
                signal_label = f"{uploaded_file.name} [window {idx}]"
                st.info(f"Array shape: {data.shape} — showing window {idx}")
            else:
...
<div style="font-size:1.1rem;font-weight:600;font-family:'JetBrains Mono',monospace;color:{COLORS['text']};">{total_seconds:.1f}s</div>
</div>
<div>
<div class="cs-label">Sample Rate</div>
<div style="font-size:1.1rem;font-weight:600;font-family:'JetBrains Mono',monospace;color:{COLORS['text']};">{fs_input} Hz</div>
</div>
<div>
<div class="cs-label">Total Samples</div>
<div style="font-size:1.1rem;font-weight:600;font-family:'JetBrains Mono',monospace;color:{COLORS['text']};">{len(signal):,}</div>
</div>
</div>""", unsafe_allow_html=True)

    # ── SLIDING WINDOW DEMO ───────────────────────────────────────────────
    st.markdown(f'<div class="cs-label" style="margin-top:0.5rem;">Sliding Window Analysis</div>',
                unsafe_allow_html=True)

    n_windows  = max(1, (len(signal) - WINDOW) // (WINDOW // 2) + 1)
    total_wins = max(1, int((len(signal) - WINDOW) / (WINDOW // 2)) + 1)
    ...
    with w_col1:
        win_idx = st.slider(
            "Window position",
            min_value=0,
            max_value=max(0, total_wins - 1),
            value=0,
            step=1,
            help=f"Slide through the recording. Each window = 30s with 50% overlap.",
            label_visibility="collapsed",
        )
    with w_col2:
        st.markdown(
            f"<div style='padding:0.4rem 0;font-family:JetBrains Mono,monospace;"
            f"font-size:0.8rem;color:{COLORS['text_mid']};'>"
            f"Window {win_idx+1} / {total_wins}</div>",
            unsafe_allow_html=True,
        )
    ...
    if win_end > len(signal):
        win_end   = len(signal)
        win_start = max(0, win_end - WINDOW)

    window_sig    = signal[win_start:win_end].astype(np.float32)
    window_start_s = win_start / fs_input

    # ── PREPROCESS + FEATURES ────────────────────────────────────────────
    with st.spinner("Processing window…"):
    ...
    if model_choice == "XGBoost":
        ...
        if mdl is None:
            ...
            method_note = "HRV heuristic (fallback)"
        else:
            label, prob, threshold = predict_xgb(mdl, features)
            method_note = f"XGBoost — {weights_path}"
            imp_fig = plot_feature_importance_xgb(mdl["model"] if isinstance(mdl, dict) else mdl)
        ...
        if mdl is None:
            ...
            imp_fig = plot_feature_importance_cb(mdl["model"] if isinstance(mdl, dict) else mdl)
    ...
    if is_afib:
        ...
        <div class='cs-alert cs-alert-afib'>
          ...
          <div>
            <div style='font-weight:700; font-size:0.95rem; color:{COLORS["danger"]};
                        font-family:"Sora",sans-serif;'>
              Atrial Fibrillation Detected
            </div>
            <div style='font-size:0.78rem; color:{COLORS["text_mid"]}; margin-top:3px;'>
              AFib probability: <strong>{prob*100:.1f}%</strong> —
              Consult a physician immediately.
              &nbsp;·&nbsp; Threshold: <strong>{threshold*100:.0f}%</strong>
              &nbsp;·&nbsp; Window: {win_start/fs_input:.1f}s–{win_end/fs_input:.1f}s
            </div>
          </div>
        </div>""", unsafe_allow_html=True)
    else:
        ...
        <div class='cs-alert cs-alert-normal'>
          ...
          <div>
            <div style='font-weight:700; font-size:0.95rem; color:{COLORS["success"]};
                        font-family:"Sora",sans-serif;'>
              Normal Sinus Rhythm
            </div>
            <div style='font-size:0.78rem; color:{COLORS["text_mid"]}; margin-top:3px;'>
    ...
    with ecg_col:
        st.plotly_chart(
            plot_ecg(proc, peaks, fs=fs_input,
                     title=f"ECG · Window {win_idx+1}/{total_wins} · "
                           f"{win_start/fs_input:.1f}s–{win_end/fs_input:.1f}s · {signal_label}",
                     is_afib=is_afib,
    ...
    # ── TABS ─────────────────────────────────────────────────────────────
    tabs = st.tabs(["💓  RR Tachogram", "🌀  Poincaré",
                    "📊  HRV Features", "🌲  Feature Importance"])
    ...
    with tabs[1]:
        if len(rr_ms) >= 4:
            st.plotly_chart(plot_poincare(rr_ms, is_afib=is_afib),
                            use_container_width=True)
        else:
    ...
    with tabs[2]:
        ...
        with left:
            st.markdown(f'<div class="cs-label">HRV Feature Values</div>',
                        unsafe_allow_html=True)
            ...
            with st.expander("🔍 Score breakdown (HRV heuristic)", expanded=False):
                for k, v in reasons.items():
                    ...
                    st.markdown(
                        ...
                        f"<div style='background:{COLORS['border']};border-radius:4px;height:6px;margin-bottom:8px'>"
    ...
    with tabs[3]:
        if imp_fig:
            ...
            st.caption("Higher = more influential in the AFib/Normal decision.")
        else:
            st.markdown(f"""
            <div class='cs-card'>
              <div style='font-size:0.85rem; color:{COLORS["text_mid"]}; line-height:1.6;'>
                Feature importance is available when XGBoost or CatBoost is loaded.
              </div>
            </div>""", unsafe_allow_html=True)
