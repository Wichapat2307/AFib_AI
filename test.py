...
from sklearn.metrics import (
    ...
    precision_score, fbeta_score, accuracy_score,
...
if baseline_path.exists():
    ...
    print(f"  AUROC={r['auroc']:.4f}  Acc={r['accuracy']:.4f}  Recall={r['recall']:.4f}  F2={r['f2']:.4f}")
else:
...
from sklearn.metrics import (
    roc_auc_score, f1_score, recall_score,
    precision_score, fbeta_score, accuracy_score, confusion_matrix
)
from sklearn.impute import SimpleImputer
Path("results").mkdir(exist_ok=True)
...
"""
    ...
    if proc.returncode == 0:
        ...
        print(f"  AUROC={xgb_result['auroc']:.4f}  Acc={xgb_result['accuracy']:.4f}  Recall={xgb_result['recall']:.4f}  F2={xgb_result['f2']:.4f}")
    else:
...
print("="*83)
print(f"{'Model':<15} {'AUROC':>7} {'Acc':>7} {'Recall':>7} {'Spec':>7} {'F1':>7} {'F2':>7} {'Prec':>7}")
print("-"*83)
for r in sorted(all_results, key=lambda x: x["auroc"], reverse=True):
    print(f"{r['model']:<15} {r['auroc']:>7.4f} {r['accuracy']:>7.4f} {r['recall']:>7.4f} "
...
if n_models > 0:
    ...
    for i, r in enumerate(all_results):
        ...
        ax.set_title(f"{r['model']}  (Acc={r['accuracy']:.3f})", color="white", fontsize=11, fontweight="bold")
        ax.tick_params(colors="#7a9bb8")
